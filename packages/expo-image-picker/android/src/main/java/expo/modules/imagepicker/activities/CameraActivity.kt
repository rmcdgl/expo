...
import expo.modules.imagepicker.toContentUri

class CameraActivity : ComponentActivity() {
  private lateinit var options: ImagePickerOptions

  private val cropperLauncher = registerForActivityResult(CropImageContract(contentResolver, options, true)) { result ->
    when (result) {
  ...
  private val cameraLauncher = registerForActivityResult(CameraContract(contentResolver)) { result ->
    when (result) {
      is ImagePickerContractResult.Cancelled -> finishCancelled()
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    val mediaFile = createOutputFile(this.cacheDir, options.mediaTypes.toFileExtension())
    val uri = mediaFile.toContentUri(this)
    cameraLauncher.launch(options.toCameraContractOptions(uri))
  }
